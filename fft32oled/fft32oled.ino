/*
 *ssd1306库文件(内包含nano_engine文件)： https://github.com/lexus2k/ssd1306/blob/master
 *fix_fft库文件在IDE-管理库里面直接搜索名称。
OLED可视化音乐频谱
 32段分频0.96寸OLED液晶可视化细条频谱灯
  实验接线: max9814接A0
  oled模块    Ardunio Uno
  GND---------GND接地线
  VCC---------5V 接电源
...
#include <fix_fft.h>                  
#include <ssd1306.h>                  
#include <nano_engine.h>  //包含在ssd1306库中  
...
uint8_t data[64], buff[32];                                     // used to store FFT input/output and past data
