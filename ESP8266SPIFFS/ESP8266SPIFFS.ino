...
String folder_name = "/";         //被读取的文件夹

//const char FileName="<!DOCTYPE html><html lang='zh-CN'><head><meta charset='UTF-8'><meta http-equiv='X-UA-Compatible' content='IE=edge'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>SPIFFS目录文件预览</title></head><body><center><a href='https://mp.csdn.net/?spm=1000.2115.3001.4503' target='_blank'><img src='/img/J200.jpg'alt='ESP8266物联网开发，DIY创作'></a><h1>ESP8266 SPIFFS 目录文件预览</h1><ul><li> ";
  
//  const char FileName2 = "</li></ul></center></body></html>";
...
/*********填写wifi信息**********/
 ...
 esp8266_server.on("/dir", find_name);      //创建查看SPIFFS文件目录页面
  if (SPIFFS.begin())
  { // 启动闪存文件系统
...
//检测SPIFS中是否有相同名称的文件，如果有就移除就从闪存中删除file_name文件
  if (SPIFFS.remove(filename)){    
    ...
    Serial.print(filename);
    Serial.println(F("闪存文件系统中不存在此文件"));
  }                       
...
String File_Name="";
  ...
  Dir dir = SPIFFS.openDir(folder_name);  // 建立“目录”对象  
  while (dir.next()) {  // dir.next()用于检查目录中是否还有“下一个文件”
    Serial.println(dir.fileName()); // 串口输出文件名
    File_Name +=dir.fileName(); // 输出文件名
  }
...
void find_name(){
  String FileDir =F("<!DOCTYPE html><html lang='zh-CN'><head><meta charset='UTF-8'><meta http-equiv='X-UA-Compatible' content='IE=edge'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>SPIFFS目录文件预览</title></head><body><center><a href='https://mp.csdn.net/?spm=1000.2115.3001.4503' target='_blank'><img src='/img/J200.jpg'alt='ESP8266物联网开发，DIY创作'></a><h1>ESP8266 SPIFFS 目录文件预览</h1><ul><li>");
  String  NewFile ="<li>";
   Dir dir = SPIFFS.openDir(folder_name);  // 建立“目录”对象  
  while (dir.next()) {  // dir.next()用于检查目录中是否还有“下一个文件”
    NewFile +=dir.fileName(); // 输出文件名
  ...
  FileDir +=NewFile;
  FileDir +=F("</li></ul><br><a href='/'>[ 返回主页 ]</a><br><br></center></body></html>");
   esp8266_server.send(200, "text/html", FileDir);   // NodeMCU将调用此函数。
}
