      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println(".button { background-color: #195B6A; border: none; color: white; padding: 16px 40px;");//关-按键背景颜色
            client.println("text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}");
            client.println(".button2 {background-color: #dd0000;}</style></head>");//开-按键背景颜色
            
            // Web Page Heading
            client.println("<body><h1>ESP8266 IO引脚按键控制页面</h1>");
            
            // Display current state, and ON/OFF buttons for GPIO 5  
            client.println("<p>GPIO 5 - 状态： " + output5State + "</p>");
