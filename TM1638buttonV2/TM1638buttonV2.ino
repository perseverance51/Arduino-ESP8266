...
unsigned char TM1638_Read(void)                                        //读数据函数
{
        ...
        pinMode(DIO,INPUT);//读数据时设置DIO为输入
        for(i=0;i<8;i++)
        {
              buttons >>=1;
            digitalWrite(CLK,LOW);
...
unsigned char Read_key(void)
{
       ...
       shiftOut(DIO, CLK, LSBFIRST, BUTTONS);//读键扫数据 命令
        pinMode(DIO, INPUT);//发完，命令，设置DIO为输入模式
         delayMicroseconds(2);//至少延时1微秒以上
        for(i=0;i<4;i++)               
          {
           // key_val[i] =TM1638_Read();
       key_val[i] = shiftIn(DIO, CLK,LSBFIRST);
...
{
        Write_COM(0x44);//采用固定地址写数据命令
        digitalWrite(STB,LOW);
//        TM1638_Write(0xc0|addr);
//        TM1638_Write(DATA);
  shiftOut(DIO, CLK, LSBFIRST, 0xC1 + (addr << 1));
...
//          TM1638_Write(0x00);
 shiftOut(DIO, CLK, LSBFIRST, 0x00);//读键扫数据 命令
        digitalWrite(STB,HIGH);
}
