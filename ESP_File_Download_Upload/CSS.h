#include <SD.h> 
#include <SPI.h>
extern String listDir(fs::FS &fs, const char * dirname, uint8_t levels);
/****************Http页眉************************/
void append_page_header() {
  ...
  webpage += F("</style></head><body><h1>SD卡文件管理系统"); webpage += String(ServerVersion) + "</h1>";
}
/****************Http页脚************************/
String Read_String(int a, int b);
void append_page_footer(){ // Saves repeating many lines of code for HTML page footers
  String filelist = listDir(SD, "/", 0);//获取SD卡根目录文件列表
  webpage += F("<ul>");
  ...
  webpage += F("<li><a href='#'>删除</a></li>"); 
  webpage += F("<li><a href='#'>查看</a></li>"); 
  webpage += F("</ul>");
 ...
 // webpage += String(char(byte(0x64/2)))+String(char(byte(0x60>>1)))+String(char(byte(0x62>>1)))+String(char(0x70>>1))+"</footer>";//年
  webpage += F("</body></html>");
}
