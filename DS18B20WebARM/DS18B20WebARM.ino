...
<!DOCTYPE HTML><html><head>
  ...
  <h3>当前温度值：<span style="color: red;  font-size: 25px;">%TEMPERATURE% &deg;C</span></h3>
  <h2 style="color: green; " >ESP32/ESP8266 温度上限报警触发设置</h2>
  <form action="/get">
    温度阈值设置： <input type="number" step="0.1" name="threshold_input" value="%THRESHOLD%" required><br>
    报警触发 <input type="checkbox" name="enable_arm_input" value="true" %ENABLE_ARM_INPUT%><br><br>
...
bool triggerActive = false;//报警触发标志位
...
const int output = 2;//板载led灯

// GPIO where the DS18B20 is connected to
const int oneWireBus = 4;     //esp8266:GPIO4--D2
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    // 检测温度超过设置报警温度，触发板载led灯亮
    if(temperature > inputMessage.toFloat() && inputMessage2 == "true" && !triggerActive){
      String message = String("Temperature above threshold. Current temperature: ")  + String("当前温度值：")+
    ...
    // Check if temperature is below threshold and if it needs to trigger output
      ...
      String message = String("Temperature below threshold. Current temperature: ") + 
                            String("当前温度值：") + String(temperature) + String(" C");
      Serial.println(message);
      triggerActive = false;
      digitalWrite(output, LOW);
    }
